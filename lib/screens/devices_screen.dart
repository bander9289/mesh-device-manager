                ...
                Consumer<DeviceManager>(builder: (context, mgr, _) {
                  ...
                  return IconButton(
                    ...
                    onPressed: !enabled
                        ...
                        : () async {
                            if (kDebugMode) {
                              debugPrint('DevicesScreen: Trigger pressed - selectionMode=$_selectionMode, selectedCount=${_selectedMacs.length}');
                            }
                            ...
                            if (_selectionMode) {
                              ...
                              if (_selectedMacs.length == 1) {
                                ...
                                if (mgr.meshClient is PlatformMeshClient) {
                                  ...
                                  final device = mgr.devices.firstWhere((d) => _selectedMacs.contains(d.macAddress));
                                  final unicastAddr = device.unicastAddress;
                                  
                                  if (kDebugMode) {
                                    debugPrint('DevicesScreen: UNICAST MODE - Sending to ${device.identifier} (${device.macAddress})');
                                    debugPrint('  Calculated unicast address: 0x${unicastAddr.toRadixString(16)}');
                                  }
                                  
                                  final success = await (mgr.meshClient as PlatformMeshClient).sendUnicastMessage(unicastAddr, true);
                              ...
                              final mgr = context.read<DeviceManager>();
                              final macs = mgr.devices.where((d) => _selectedMacs.contains(d.macAddress)).map((d) => d.macAddress).toList();
                              ...
                              if (affected < 0) {
                                await showDialog<void>(
                                  ...
                                  builder: (ctx) => AlertDialog(content: const Text('No native mesh plugin found and GATT fallback failed; install the native plugin or test on a desktop with Mock enabled.')),
                                );
                                ...
                                await showDialog<void>(
                                  context: context,
                                  barrierDismissible: true,
                                  builder: (ctx) => AlertDialog(content: Text('Triggered $affected devices')),
                                );
                              ...
                              if (affected < 0) {
                                ...
                                await showDialog<void>(
                                  context: context,
                                  barrierDismissible: true,
                                  builder: (ctx) => AlertDialog(content: Text('Triggered group $affected devices')),
                                );
                                Future.delayed(const Duration(milliseconds: 1200), () {
                                  if (context.mounted && Navigator.of(context).canPop()) Navigator.of(context).pop();
                                });
                ...
                Builder(builder: (context2) {
                  ...
                  final targetLabel = _selectionMode
                      ...
                      : (_selectedGroupId == _kUnknownGroupId
                          ? 'Unknown'
                          : (() {
                              final matches = mgr2.groups.where((g) => g.id == _selectedGroupId).toList();
                              return matches.isNotEmpty ? matches.first.name : 'Group: ${_selectedGroupId.toRadixString(16).toUpperCase()}';
                            })());
                  return Padding(padding: const EdgeInsets.only(right: 8), child: Text(targetLabel));
                }),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        leading: _selectionMode ? Checkbox(value: _selectedMacs.contains(device.macAddress), onChanged: (v) { setState(() { if (v == true) { _selectedMacs.add(device.macAddress); } else { _selectedMacs.remove(device.macAddress); } }); }) : Builder(builder: (context2) {
                          final mgr2 = context2.read<DeviceManager>();
                          int? colorVal;
                          if (device.groupId != null) {
                            try { colorVal = mgr2.groups.firstWhere((gg) => gg.id == device.groupId).colorValue; } catch (_) { colorVal = null; }
                          }
                          if (colorVal == null && _selectedGroupId != _kUnknownGroupId && mgr2.isGroupConfirmed(_selectedGroupId)) {
                            ...
                            if (members != null && members.contains(device.macAddress)) {
                          ...
                          return Stack(children: [Container(width: 28, height: 28, margin: const EdgeInsets.only(right: 8), decoration: BoxDecoration(shape: BoxShape.circle, color: bg),), if (device.lightOn == true) Positioned(right: 0, bottom: 0, child: Icon(Icons.lightbulb, size: 10, color: Colors.yellow[700])),]);
                        }),
                        title: Text(device.identifier),
                        subtitle: Text('${device.hardwareId == 'unknown' ? device.macAddress : device.hardwareId} • ${device.version} • ${device.rssi}dBm'),
                        trailing: Row(mainAxisSize: MainAxisSize.min, children: [ if (device.lightOn == true) const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)), const SizedBox(width: 8), Icon(Icons.battery_std, color: device.batteryPercent == 0 ? Colors.grey : (device.batteryPercent >= 50 ? Colors.green : (device.batteryPercent >= 25 ? Colors.orange : Colors.red))), ]),
                        onTap: () {
                          if (_selectionMode) {
                            setState(() {
                                if (_selectedMacs.contains(device.macAddress)) { _selectedMacs.remove(device.macAddress); } else { _selectedMacs.add(device.macAddress); }
                            });
                            return;
                          }
                          final dm = context.read<DeviceManager>();
      ...
      child: Row(children: [
        ...
        Row(children: [
          ...
          IconButton(onPressed: selected.isEmpty ? null : () async {
            ...
            if (kDebugMode) {
              debugPrint('UI: Assign Selected to Group ${_selectedGroupId != _kUnknownGroupId ? _selectedGroupId.toRadixString(16) : 'Default'}; selected=${selected.map((d) => d.macAddress).join(', ')}');
            }
            // Assign to currently selected group if it's not Unknown; otherwise default group
            final targetGroupId = _selectedGroupId != _kUnknownGroupId ? _selectedGroupId : 0xC000;
            MeshGroup? targetGroup;
            try { targetGroup = manager.groups.firstWhere((g) => g.id == targetGroupId); } catch (_) { targetGroup = null; }
            targetGroup ??= manager.createGroupFromDevices([], name: targetGroupId == 0xC000 ? 'Default' : null, groupAddress: targetGroupId);
            manager.moveDevicesToGroup(selected, targetGroup.id);
            setState(() { _selectionMode = false; _selectedMacs.clear(); });
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Assigned ${selected.length} devices to ${targetGroup.name}')));
            }
          }, icon: const Icon(Icons.group_add), tooltip: 'Assign Selected to Group'),
          const SizedBox(width: 8),
          IconButton(onPressed: selected.isEmpty ? null : () async {
            final mgr = context.read<DeviceManager>();
            
            // SPECIAL CASE: If exactly one device selected, send unicast to test status responses
            if (selected.length == 1 && mgr.meshClient is PlatformMeshClient) {
              ...
              final success = await (mgr.meshClient as PlatformMeshClient).sendUnicastMessage(
                unicastAddr, 
                true,
                proxyMac: device.macAddress, // Use this device as proxy
              );
              if (!context.mounted) return;
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
            ...
            if (!context.mounted) return;
            if (affected < 0) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Trigger failed: native plugin not present and GATT fallback failed')));
            } else {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Triggered $affected devices')));
            }
            setState(() { _selectionMode = false; _selectedMacs.clear(); });
          }, icon: const Icon(Icons.flash_on), tooltip: 'Trigger Selected'),
          const SizedBox(width: 8),
